...
function ami = avg_mi_old(A,B,factors, M, idxs) 
...
%    ami = mean(moran_indices(A(1:M,:),B(1:M,:),factors)); 
  ...
  figure(1); hold on; scatter(B(interval,4), B(interval,3)); xlabel(sprintf('Population Group Size %d',M),'FontSize', 18); 
  axis([-130,-60,25,50]);  % zoom out to US map
  
  cami = mean(moran_indices(A(interval,:),B(interval,:),factors)); 
